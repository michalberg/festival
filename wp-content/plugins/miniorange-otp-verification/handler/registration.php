...
class MoRegistrationHandler
{
	...
	function _register_customer($post)
	{
		...
		if( strlen( $password ) < 6 || strlen( $confirmPassword ) < 6)
		{
			...
			do_action("mo_registration_show_error_message");
			return;
		}
		...
		if( $moUtility->mo_check_empty_or_null( $email ) || $moUtility->mo_check_empty_or_null( $password )
				|| $moUtility->mo_check_empty_or_null( $confirmPassword ) )
		{
	...
	{
		$customer = new MocURLOTP();
		$content  = json_decode($customer->mo_send_otp_token($auth_type,$email,$phone), true);
	...
	function _validate_otp($post)
	{
		...
		if( $moUtility->mo_check_empty_or_null( $otp_token ) )
		{
		...
		if(strcasecmp($content['status'], 'SUCCESS') == 0)
		{
			...
			else if(strcasecmp($customerKey['status'], 'SUCCESS') == 0)
			{
				...
				update_option('mo_otp_plugin_version',1.9);
				do_action("mo_registration_show_success_message");
	...
	function _send_phone_otp_token($post)
	{
		...
		{
			update_option('mo_customer_validation_message','There was an error in sending sms. Please Check you phone number.');
			update_option('mo_customer_validation_registration_status','MO_OTP_DELIVERED_FAILURE');
			do_action("mo_registration_show_error_message");
		}
	}
	...
	function _reset_password()
	{
		$customer = new MocURLOTP();
		$email 	  = get_option('mo_customer_validation_admin_email');
		$forgot_password_response = json_decode($customer->forgot_password($email));
