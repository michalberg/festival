jQuery(document).ready(function () {	
    ...
    $('input[name=mo_customer_validation_ninja_form_enable_type]').change(function(){
        $('input[name^="ninja_form"]').each(function() {
            $(this).val('');
        });
    });
    ...
    $("#ov_settings_button").click(function() {
        ...
        //wp default form
        if(!$("#wp_default").is(':checked')){
            if($('input[name=mo_customer_validation_wp_default_enable_type]').is(':checked'))
                $('input[name=mo_customer_validation_wp_default_enable_type]').prop("checked", false);
        }else{
            if( !$('input[name=mo_customer_validation_wp_default_enable_type]').is(':checked')){
                $('#error_message').val($('#error_message').val()+'Please choose a Verification Method for WordPress Default Registration Form.<br/>');
                $("#wp_default").prop("checked",false);
            }
        }
        ...
        //buddypress form
         if(!$("#bbp_default").is(':checked')){
            ...
            if($('input[name=mo_customer_validation_bbp_enable_type]').is(':checked')){
                ...
                }else if($('#bbp_rcon').is(':checked')){
                    if ($('#bbp_rcon_server_addr').val() === '' || $('#bbp_rcon_server_port').val() === ''){
                        $("#bbp_default").prop("checked", false);
                        $('#bbp_rcon').prop("checked", false);
                        $('input[name=bbp_rcon_server_addr]').val('');
                        $('input[name=bbp_rcon_server_port]').val('');
                        $('#error_message').val($('#error_message').val()+'Please Enter your Server Details.<br/>');
                    }
                }else{
                     $('input[name=bbp_phone_field_key]').val('');
                }
       ...
       if(!$("#uultra_default").is(':checked')){
		...
		//pie registration
        if(!$("#pie_default").is(':checked')){
            if($('input[name=mo_customer_validation_pie_enable_type]').is(':checked'))
                $('input[name=mo_customer_validation_pie_enable_type]').prop("checked", false);
            $('input[name=pie_phone_field_key]').val('');
       }else{
            if($('input[name=mo_customer_validation_pie_enable_type]').is(':checked')){
                if($('#pie_phone').is(':checked')){
                    if ($('#pie_phone_field_key').val() === ''){
                        $("#pie_default").prop("checked", false);
                        $('#pie_phone').prop("checked", false);
                       $('input[name=pie_phone_field_key]').val('');
                        $('#error_message').val($('#error_message').val()+'Please Enter the Meta Key of the phone field.<br/>');
                    }
                    else{
                    	$('input[name=pie_phone_field_key]').val($('#pie_phone_field_key').val());
                    }
                }else if($('#pie_both').is(':checked')){
			    	if ($('#pie_phone_field_key1').val() === ''){
			            $("#pie_default").prop("checked", false);
			            $('#pie_both').prop("checked", false);
			            $('input[name=pie_phone_field_key]').val('');
			            $('#error_message').val($('#error_message').val()+'Please Enter the Meta Key of the phone field.<br/>');
			        }
			        else{
			        	$('input[name=pie_phone_field_key]').val($('#pie_phone_field_key1').val());
			        }
                }else{
                     $('input[name=pie_phone_field_key]').val();
                }
                   ...
                   $("#pie_default").prop("checked", false);
                   $('#error_message').val($('#error_message').val()+'Please choose a Verification Method for Pie Registration Form.<br/>');
            }
        }
        ...
        //ninja form
        if(!$("#ninja_form").is(':checked')){
            if($('input[name=mo_customer_validation_ninja_form_enable_type]').is(':checked'))
                $('input[name=mo_customer_validation_ninja_form_enable_type]').prop("checked", false);
            $('input[name^="ninja_form"]').each(function() {
                $(this).val('');
            });
        }else{
            if(!$('input[name=mo_customer_validation_ninja_form_enable_type]').is(':checked')){
                $("#ninja_form").prop("checked", false);
                $('#error_message').val($('#error_message').val()+'Please choose a Verification Method for Ninja Form.<br/>');
            }
        }

        //tml form
        if(!$("#tml_default").is(':checked')){
            if($('input[name=mo_customer_validation_tml_enable_type]').is(':checked'))
                $('input[name=mo_customer_validation_tml_enable_type]').prop("checked", false);
        }else{
            if( !$('input[name=mo_customer_validation_tml_enable_type]').is(':checked')){
                $('#error_message').val($('#error_message').val()+'Please choose a Verification Method for Theme My Login Registration Form.<br/>');
                $("#tml_default").prop("checked",false);
            }
        }
	 ...
	 $(".mo_registration_help_title").click(function(e){
    	e.preventDefault();
    	$(this).next('.mo_registration_help_desc').slideToggle(400);
    });
	
	$("#lk_check1").change(function(){
        if($("#lk_check2").is(":checked") && $("#lk_check1").is(":checked")){
            $("#activate_plugin").removeAttr('disabled');
        }
    });
	
    $("#lk_check2").change(function(){
        if($("#lk_check2").is(":checked") && $("#lk_check1").is(":checked")){
            $("#activate_plugin").removeAttr('disabled');
        }
    });

    $("#mo_search").on("keyup", function() {
        var value = $(this).val();
        $("#mo_forms tr").each(function(index) {
            if (index !== 0) {
                $row = $(this);
